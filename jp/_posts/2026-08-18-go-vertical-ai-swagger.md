...
> **TL;DR**
> 1. GoとGinは、バーティカルAI APIの並行処理と軽量なデプロイを支えるバックエンド基盤です。
> 2. MongoDBにはAIの原文、ドメイン情報、トークン・デバッグログを柔軟に保存し、TTLポリシーで保持期間を管理します。
> 3. Swagger/OpenAPIをコードコメントから自動生成することで、API実装とドキュメントのDocs-Driftを減らせます。
...
### Q. なぜバーティカルAI APIにMongoDBを使うのですか？

AI処理の結果には、原文テキスト、ドメイン別の構造化データ、トークン使用量、デバッグ用のRaw Responseなど、形式の異なる情報が含まれます。MongoDBはこのような半構造化された追跡ログを柔軟に保存し、インデックスやTTLによる保持期間の管理も行えます。

### Q. コードコメントからSwaggerを生成するとDocs-Driftをどう減らせますか？

APIハンドラーとリクエスト・レスポンスの仕様を同じ変更単位で管理し、`swag init`でドキュメントを再生成すれば、実装とは別に古い仕様書を手作業で更新する問題を減らせます。CIでは生成結果と認証・エラー応答もあわせて検証します。
