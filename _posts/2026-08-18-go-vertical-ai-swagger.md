...
> **TL;DR**
> 1. Go·Gin은 버티컬 AI API의 동시성 처리와 가벼운 배포에 적합한 백엔드 기반을 제공합니다.
> 2. MongoDB에는 AI 원문, 도메인 정보, 토큰·디버그 로그를 유연하게 저장하고, TTL 정책으로 보존 기간을 관리합니다.
> 3. Swagger/OpenAPI를 코드 주석에서 자동 생성하면 API 구현과 문서 사이의 Docs-Drift를 줄일 수 있습니다.
...
### Q. 버티컬 AI API에 MongoDB를 사용하는 이유는 무엇인가요?

AI 처리 결과에는 원문 텍스트, 도메인별 구조화 데이터, 토큰 사용량, 디버그용 Raw Response처럼 형태가 서로 다른 정보가 함께 저장될 수 있습니다. MongoDB는 이러한 반정형 추적 로그를 유연하게 저장하고, 필요에 따라 인덱스와 TTL 보존 정책을 적용하기에 적합합니다.

### Q. Swagger 문서를 코드 주석에서 생성하면 Docs-Drift를 어떻게 줄일 수 있나요?

API 핸들러와 요청·응답 명세를 같은 코드 변경과 함께 관리하고 `swag init`으로 문서를 다시 생성하면, 구현과 별도로 오래된 문서를 수동 관리하는 문제를 줄일 수 있습니다. 다만 CI에서 생성 결과를 검증하고, 인증·오류 응답·버전 정보까지 함께 점검해야 합니다.
